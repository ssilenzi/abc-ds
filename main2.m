...
for i = 1:length(lasa_ids)
    lid = lasa_ids(i);
sdpbmibnb = sdpsettings('solver','bmibnb','verbose',1,'usex0',1,'warmstart',1);
sdpbmibnb = sdpsettings(sdpbmibnb, ...
    'bmibnb.sdpsolver',   'mosek', ...   % SDP relaxations
    'bmibnb.lpsolver',    'mosek', ...   % LP relaxations
    'bmibnb.uppersolver', 'fmincon', ...
    'bmibnb.lowersolver', 'mosek', ...
    'bmibnb.maxiter',     500, ...
    'bmibnb.maxtime',     600, ...       % optional: wall-clock cap (sec)
    'fmincon.Algorithm',  'interior-point', ...
    'fmincon.Display',    'off');

% --- Pass options the way fvb.m expects (note the field name) ---
experiments{end+1}.options = fvbsettings( ...
    'enable_barrier', false, ...
    'epsilon', 1e-3, ...
    'init_Bc_var', false, ...
    'constraint_version', 3, ...
    'dataset', 'lasa', ...
    'dataset_opts', struct('idx', lid, 'obstacle_repr', "ellipse_axis"), ...
    'enable_regularization', false, ...
    'deg_f', 6, 'deg_V', 4, 'deg_B', 2, 'deg_B_slack', 2, ...
    'enable_extra_constraint', true, ...
    'regularization_factor', 0.01, ...
    'seed', local_seed, ...
    'sdpoptions_bmibnb', sdpbmibnb);
    experiments{end}.pre = {};
